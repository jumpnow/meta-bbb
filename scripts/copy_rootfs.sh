...
if [ ! -f ${SRCDIR}/zImage-${DTS} ]; then
	echo "DTS file not found: ${SRCDIR}/zImage-${DTS}"
	exit 1
fi 
...
if [ -b $DEV ]; then
	...
	echo "Copying zImage-${DTS} to /media/card/boot/"
	sudo cp ${SRCDIR}/zImage-${DTS} /media/card/boot/${DTS}
